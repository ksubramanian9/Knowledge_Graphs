(async () => {
  ...
  async function askOllama(n) {
    ...
    const prompt = `You are a FinOps tutor. Explain the concept "${n.id}" in depth: definition, key properties, relationships to adjacent concepts in a knowledge graph (like ${neighborsOf(n.id).slice(0,6).join(", ")}), common pitfalls, 1-2 worked examples, and when to use it. Use Markdown with code blocks and LaTeX where helpful.`;
    try {
